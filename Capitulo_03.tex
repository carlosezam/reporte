...
A continuación se ofrece un resumen de las principales ramas de la Inteligencia Artificial, a saber: Redes Neuronales, Algoritmos Genéticos, Sistemas Expertos y Lógica Difusa. Esta última que también es una rama de la IA, será tratada con más detalle en la siguiente sección (sección \ref{seccion:logicadifusa}).
\subsubsection{Redes neuronales artificiales}

\begin{wrapfigure}{r}{5.5cm}
	\vspace{-\baselineskip}
	\includegraphics[width=5cm]{Sources/neurona.png}
	\captionof{figure}{Esquema de una RNA}
	\vspace{-\baselineskip}
\end{wrapfigure}
...
Una Red de Neuronas Artificiales (en adelante, RNA) es un paradigma de procesamiento de información inicialmente \textbf{inspirado en el modo en que lo hace el cerebro}. El elemento clave de este paradigma es su estructura. Las RNA están compuestas por un cierto número de elementos de procesamiento o neuronas que trabajan al unísono para resolver un problema específico.
...
\begin{figure}[H]
	...
	\caption{Funciones de activación en MATLAB}
\end{figure}

\subsubsection{Algoritmos genéticos}
Los Algoritmos Genéticos (AG) fueron introducidos inicialmente por Holland en 1975 para \textbf{abstraer y explicar rigurosamente los procesos adaptativos de los sistemas naturales}, así como para el diseño de sistemas artificiales de software que retengan los mecanismos importantes de los sistemas naturales. Fue unos años más tarde cuando su alumno, D. Goldberg, implementó el primer AG aplicado en problemas industriales. Estas y otras aplicaciones creadas por estudiantes de Holland convirtieron los AGs en un campo suficientemente aceptado.

En un AG, las soluciones potenciales al problema se representan normalmente mediante cadenas binarias de bits (0’s y 1’s) de una longitud determinada \emph{long} que vendrá impuesta por el número de variables existentes en la solución y por el número de bits necesarios para codificarlas. Otros términos usados a menudo para denominar una solución del problema en un AG son string o estructura, y, siguiendo el vocabulario de los sistemas biológicos, \textit{cromosoma}.

Así, los cromosomas están compuestos por unidades binarias que se denominan \textit{genes}. Al valor de un gen determinado se le denomina \textit{alelo}, y a su posición en el cromosoma \textit{locus}. Al paquete genético total se le denomina \textit{genotipo}, y a la interacción del genotipo con su entorno se le denomina \textit{fenotipo}, que se traduce en la decodificación del cromosoma para la obtención de una solución alternativa (conjunto de parámetros particulares, o un punto en el espacio de búsqueda). De esta forma, podemos representar un cromosoma $c_i^t$ en una generación (iteración) determinada $t$ como:
{\large $$ c_i^t = \displaystyle (b_{i_1}^t \ldots b_{i_long}^t)$$}
Con $b_{i_j}^t$ $\in$ \{0, 1\}, $j$ = 1,$\ldots$, $long$. El término \textit{individuo} es frecuentemente utilizado para referirse al conjunto de información genotipo-fenotipo adecuación. Así, podemos representar un individuo $X_i^t$ en una generación $t$, como la terna:
...
El área de sistemas expertos es una aproximación muy exitosa a la solución de problemas clásicos de IA en la programación de inteligencia. El profesor Edward Feigenbaum de la Universidad de Stanford, pionero en la tecnología de los sistemas expertos, los ha definido como “un programa de computación inteligente que usa el conocimiento y procedimientos de inferencia para resolver problemas que son lo suficientemente difícil como para requerir significativa experiencia humana para su solución, es decir, un sistema experto es un sistema de cómputo que \textbf{emula la habilidad de tomar decisiones de un especialista humano}.
...
\end{center}
Aunque se trata de un ejemplo muy simple, se han construido muchos sistemas expertos significativos expresando el conocimiento de especialistas mediante reglas.

Los sistemas expertos se han aplicado casi a todos los campos del conocimiento, a continuación en la tabla \ref{table:se} se mencionan algunos de ellos.
...
El concepto de conjunto difuso fue propuesto por Zadeh(1965): ``Un conjunto difuso es una colección de objetos con un \textit{grado de membresía} continuo. Un conjunto tal, es caracterizado por una \textit{función de membresía} que asigna a cada objeto un grado de membresía en un rango de 0 a 1.''.
...
Sin embargo, los conceptos manejados por el ser humano como \textit{frío} y \textit{caliente}, tienen una transición gradual. Así, un conjunto difuso, lidia con la vaguedad inherente de estos conceptos, conteniendo a los elementos sólo con un cierto grado de pertenencia.
...
Sea $X$ el \emph{Universo del discurso}, y sus elementos se denotan como $x$; En la teoría clásica de conjuntos se define un conjunto $C$ sobre $X$ mediante una \emph{función característica} de $C$ como $f_c$ .
...
Es bien conocido que la \textit{Teoría de conjuntos}, el \textit{Álgebra booleana} y la \textit{Lógica tradicional} son isomorfas, bajo transformaciones adecuadas. Esto significa que tienen una estructura subyacente similar, y que por tanto, las definiciones que se hagan en una de las tres teorías se pueden llevar a las otras dos, mediante las transformaciones adecuadas.
...
En donde se ha utilizado el operador \textit{min} parara efectuar la intersección de los dos conjuntos, pero en general podría haberse utilizado cualquier \textit{norma T}.
...
En forma análoga, al operador lógico $OR$ puede hacerse corresponder a una \textit{norma S}, mientras al operador lógico $NOT$ puede hacerse corresponder con el complemento.
...
\begin{enumerate}
	...
	\item \textsl{Monotonicidad:} $T(a,b) \leq T(c,d)$ si $a \leq c$ y $b \leq d$
...
\begin{enumerate}
	...
	\item \textsl{Monotonicidad:} $S(a,b) \leq S(c,d)$ si $a \leq c$ y $b \leq d$
...
\begin{wrapfigure}{r}{5cm}
	...
	\draw[blue] (5.6,0.3) node[above] {$a$};
	...
	\label{fig:gaussmf}
\end{wrapfigure}
La función gaussiana es una función simétrica que juega el papel de la función triangular, solo que esta pertenece al grupo de las \textit{funciones con derivada continua}. Si observamos la figura \ref{fig:gaussmf} podemos ver como la gráfica asemeja un triángulo suavizado, es decir, los vértices no tienen cambios abruptos sino graduales.
...
Donde
{\setlength{\baselineskip}{0.7\baselineskip}\begin{description}
	\item $x_0$ determina el centro o núcleo,
...
\begin{wrapfigure}{r}{5cm}
	\vspace{-\baselineskip}
		\begin{tikzpicture}[domain=0:8, xscale=0.5]
			...
			\draw[blue] (4,1) node[above] {$x_0$};
			\draw[blue] (4,0.9) node[below] {$b$};
			\draw[blue] (5.8,0.3) node[above] {$a$};
		\end{tikzpicture}
		\captionsetup{margin=0.2cm,justification=centering}	
...
Su definición matemática es la siguiente:
\begin{displaymath}
f(x;a,b,x_0)= \frac{1}{1 + \left(  \frac{x-x_0}{b}  \right)^{2a}}
\end{displaymath}
Donde
{\setlength{\baselineskip}{0.7\baselineskip}\begin{description}
	\item $x_0$ determina el centro,
	\item $a$ determina la pendiente y,
	\item $b$ determina el ancho del núcleo.
\end{description}}
...
\begin{wrapfigure}{r}{5cm}
	...
	\begin{tikzpicture}[domain=0:8, xscale=0.5, yscale=1]
		...
		\draw[blue] (4,0.3) node[above] {$x_0$};
		\draw[blue] (2.5,0.9) node[below] {$a$};
	\end{tikzpicture}
	\captionsetup{margin=0.2cm,justification=centering}	
...
Una sigmoidal, es una función abierta y con derivada continua. Su gráfica recuerda la forma de un escalón. Tiene un parámetro $a$ que determina su pendiente, es decir, determina la suavidad de la transición entre los valores de membresía 0 a 1. Además el parámetro $a$ también determina si la función abrirá por la izquierda o por la derecha. Un valor positivo de $a$ genera un gráfica abierta por la derecha
...
{\setlength{\baselineskip}{0.7\baselineskip}\begin{description}
	...
	\item $G$ es la regla sintáctica que genera los términos en $T(x)$.
...
\begin{figure}[H]
	...
	\caption{Diagrama esquemático de un sistema de inferencia difusa}
	\label{fig:digfis}
\end{figure}
...
Los conjuntos difusos son caracterizados mediante funciones de membresía ajustadas a las necesidades del sistema. La figura \ref{fig:fuzzify} muestra la interpretación grafica de la fuzzificación.
...
El objetivo de las reglas difusas es capturar el conocimiento empírico del experto. Al conjunto de reglas se le denomina \textit{Base de Conocimientos}.
...
En los sistemas de reglas clásicos, si el antecedente es cierto, el consecuente también lo es. En un sistema difuso donde el antecedente es difuso, todas las reglas se ejecutan parcialmente, por lo que el consecuente es cierto o falso pero solo en cierto grado.

\subsubsection*{Inferencia}
En la segunda etapa se proponen reglas lingüísticas que servirán para inferir la salida del sistema.
El grado de pertenencia de cada una de las variables se evalúa en un conjunto de
reglas de inferencia. Dichas reglas de inferencia son determinadas con ayuda de un experto. El conjunto
...
proveniente de la consecuencia de las reglas de inferencia (\textit{conjunto consecuencia}), a un valor nítido o real. Para hacer
esto, previamente se sintonizaron las funciones de membresía de cada una de las salidas con el fin de
obtener un valor cuantificable útil.

Entre los diversos métodos que existen para obtener la salida del sistema, uno de los más usados es el método de \textit{centroide} que consiste en determinar el centro de gravedad del área resultante bajo la curva del conjunto resultado.

\begin{figure}[H]
	\centering
	\begin{tikzpicture}[scale=1.2]
	
	\filldraw [fill=blue!20!white] (0,0) -- (1,0) -- (1.5,1) -- (2.5,1) -- (3,2) -- (4,2) -- (4.25,1.4) -- (5.25,1.4) -- (5.9,0) -- cycle;
	\filldraw [gray] (3.7,0.6) circle [radius=4pt];
	
	\draw[->] (0,0) -- (7,0) 	node[right] {$x$} 		coordinate (x axis);
	\draw[->] (0,0) -- (0,2) 	node[above]	{$\mu(x)$} 	coordinate (y axis);
	
	\draw [arrows = {-Stealth[inset=0pt, length=10pt]},gray,dashed,very thick] (3.7,0.4) -- (3.7,-0.5);
	
	\draw[font=\scriptsize] (3.7,-0.5) node[below] {$Z_0$};
%	\draw[font=\scriptsize] (2.5,0.45) node[below] {$C'$};
	\end{tikzpicture}
	\captionsetup{margin=1cm,justification=centering}	
	\caption{Defuzzificación por \textit{centroide} de un conjunto resultado C'}
	\label{fig:inference}
\end{figure}
