...
\subsection{Principales Ramas de la IA}\label{cap:ramasdelaia}
...
\subsection{Funciones de Membresía}
...
{\setlength{\baselineskip}{0.7\baselineskip}\begin{description}
	...
	\item $\sigma$ determina el ancho o la pendiente.
\end{description}}
...
Un sistema de inferencia difuso (\textit{FIS}) consta de tres etapas para obtener el resultado deseado. La primera etapa se basa en
un proceso donde las variables tienen un grado de incertidumbre metalingüístico.
Por lo tanto, el rango de valores (\textit{universo de discurso}) de cada variable puede clasificarse por conjuntos difusos, por ejemplo
\textit{baja, media, alta}.
Cuando los sensores miden las variables, los valores pasan a un proceso de \textit{fuzzificación}
que consiste en pasar dichos valores a un rango de pertenencia entre cero (0) y uno (1). Se busca
determinar en qué grado el valor que se está adquiriendo pertenece a un conjunto difuso. Los conjuntos
difusos son caracterizados mediante funciones de membresía, las cuales están sintonizadas al punto de
operación adecuado para el funcionamiento del sistema.

En la segunda etapa se proponen reglas lingüísticas (\textit{inferencia}) que servirán de guía para que el
sistema se comporte de manera más adecuada, idónea o deseada según el modelo de referencia o los
objetivos del usuario. El grado de pertenencia de cada una de las variables se evalúa en un conjunto de
...
obtener un valor cuantificable. La figura 1.3 muestra el diagrama esquemático del controlador
difuso.
